    ...
    return (
        <header className="mt-6 p-4 sm:flex sm:items-center sm:justify-center max-[640px]:flex max-[640px]:flex-col gap-7 ">
            <div className="flex justify-between">
                ...
                {/* hamburger icon */}
                <GiHamburgerMenu onClick={() => setShowCategory(true)} className="swap-off fill-current min-[640px]:hidden text-3xl cursor-pointer" />
            </div>
            ...
            {showCategory && (
                <div className="fixed top-0 left-0 w-full h-full bg-white z-50 sm:hidden">
                    <div className="min-w-[320px] w-full h-full bg-white p-4 overflow-y-auto flex flex-col gap-5">
                        {/* close icon */}
                        <AiOutlineClose onClick={() => setShowCategory(false)} className="swap-on fill-current min-[640px]:hidden text-3xl cursor-pointer" />
                        {
                            categories.map((category, i) => <p className="text-2xl font-bold cursor-pointer" key={i}>{category}</p>)
