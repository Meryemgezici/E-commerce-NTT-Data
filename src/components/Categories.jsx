...
const Categories = () => {
    ...
    return (
        <>
            {!showCategory ? <div>
                {selectCate === "" ? <button onClick={(e) => { handleClick(e) }} className="flex justify-center items-center gap-4 rounded-r absolute bg-[#F4F5F6] right-[153px] top-[1px]  px-3 py-[6px] h-[38px] min-w-[132px] outline-none text-sm max-[640px]:hidden">
                    <p>Categories</p>
                    <MdKeyboardArrowDown className="text-3xl" />
                </button> : <button onClick={(e) => { handleClick(e) }} className="flex justify-center items-center gap-4 rounded absolute bg-white border border-[#0059BC] right-[153px] top-[1px]  px-3 py-[6px] h-[38px] min-w-[132px]  text-sm max-[640px]:hidden">
                    <p>{selectCate}</p>
                    <MdKeyboardArrowDown className="text-3xl" />
                </button>}
            </div>
                : <div className="flex flex-col gap-3 absolute right-[132px]  top-[1px]">
                    <button onClick={(e) => { handleClick(e) }} className="flex items-center justify-between gap-4 rounded bg-white min-w-[132px]   px-3 py-[6px] h-[38px]  border border-[#0059BC] text-sm ">
